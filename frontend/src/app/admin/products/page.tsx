      ...
      <main className="flex-1 lg:mr-64 p-4 lg:p-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
            ...
            <Button onClick={() => router.push('/admin/products/new')} icon="mdi:plus">
              محصول جدید
            </Button>
          </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border bg-surface-raised">
                    ...
                    <th className="text-right px-4 py-3 text-sm font-medium text-text-secondary">محصول</th>
                    <th className="text-center px-4 py-3 text-sm font-medium text-text-secondary hidden md:table-cell">قیمت</th>
                    <th className="text-center px-4 py-3 text-sm font-medium text-text-secondary hidden lg:table-cell">موجودی</th>
                ...
                <tbody>
                  {isLoading ? (
                    ...
                    data?.data?.map((product) => (
                      <tr key={product.id} className="border-b border-border hover:bg-surface-raised/50 transition-colors">
                          ...
                          <div className="flex items-center gap-3">
                            {product.thumbnail ? (
                              ...
                              <div className="w-10 h-10 rounded-lg bg-surface-raised flex items-center justify-center">
                                <Icon icon="mdi:image-off" className="w-5 h-5 text-text-muted" />
                              </div>
                            )}
                            <div>
                        ...
                        <td className="px-4 py-3 text-center">
                          <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${
                            product.is_active ? 'bg-success-light text-success' : 'bg-error-light text-error'
                          }`}>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-center gap-1">
                            <button onClick={() => router.push(`/admin/products/${product.id}`)} className="p-2 hover:bg-primary-light rounded-button text-primary" title="ویرایش">
                              <Icon icon="mdi:pencil" className="w-4 h-4" />
                            </button>
                            <button onClick={() => handleDelete(product)} className="p-2 hover:bg-error-light rounded-button text-error" title="حذف">
            ...
            {data?.meta && data.meta.totalPages > 1 && (
              <div className="flex items-center justify-between px-4 py-3 border-t border-border">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="p-2 hover:bg-surface-raised rounded-button disabled:opacity-50">
                  ...
                  {Array.from({ length: data.meta.totalPages }, (_, i) => i + 1).slice(Math.max(0, page - 3), Math.min(data.meta.totalPages, page + 2)).map((p) => (
                    <button key={p} onClick={() => setPage(p)} className={`w-10 h-10 rounded-button text-sm font-medium ${p === page ? 'bg-primary text-white' : 'text-text-secondary hover:bg-surface-raised'}`}>
                      {p}
                    </button>
                  ))}
                  <button onClick={() => setPage((p) => Math.min(data.meta.totalPages, p + 1))} disabled={page === data.meta.totalPages} className="p-2 hover:bg-surface-raised rounded-button disabled:opacity-50">
