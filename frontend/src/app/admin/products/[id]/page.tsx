        ...
        <div className="max-w-4xl mx-auto">
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="space-y-6">
              ...
              <div className="bg-surface rounded-card shadow-card p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-text-secondary">عنوان *</label>
                    <input {...register('title')} className="w-full px-4 py-2 bg-surface border border-border rounded-input focus:outline-none focus:ring-2 focus:ring-primary" />
                    {errors.title && <p className="text-sm text-error">{errors.title.message}</p>}
                  </div>

                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-text-secondary">دسته‌بندی *</label>
                    <select {...register('category_id')} className="w-full px-4 py-2 bg-surface border border-border rounded-input focus:outline-none focus:ring-2 focus:ring-primary">
                      <option value="">انتخاب کنید</option>
                      {categoriesData?.data?.map((cat) => (
                        <option key={cat.id} value={cat.id}>{cat.name}</option>
                      ))}
                    </select>
                    {errors.category_id && <p className="text-sm text-error">{errors.category_id.message}</p>}
                  </div>

                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-text-secondary">برند</label>
                    <select {...register('brand_id')} className="w-full px-4 py-2 bg-surface border border-border rounded-input focus:outline-none focus:ring-2 focus:ring-primary">
                  ...
                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-text-secondary">توضیح کوتاه</label>
                    <input {...register('short_description')} className="w-full px-4 py-2 bg-surface border border-border rounded-input focus:outline-none focus:ring-2 focus:ring-primary" />
                  </div>
                </div>
              </div>
              ...
              {/* Status */}
              <div className="bg-surface rounded-card shadow-card p-6">
                <h2 className="text-lg font-bold text-text-primary mb-6">وضعیت</h2>
                <div className="flex gap-8">
                  ...
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" checked={isPublic} onChange={(e) => setValue('is_public', e.target.checked)} className="sr-only peer" />
                    ...
                    <span className="text-sm text-text-secondary">{isPublic ? 'منتشر شده' : 'پیش‌نویس'}</span>
                  </label>
                </div>
              </div>
              ...
              <div className="bg-surface rounded-card shadow-card p-6">
                ...
                <div className="grid grid-cols-1 gap-6">
                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-text-secondary">عنوان سئو</label>
                    <input {...register('seo_title')} className="w-full px-4 py-2 bg-surface border border-border rounded-input focus:outline-none focus:ring-2 focus:ring-primary" />
                  </div>
                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-text-secondary">توضیحات سئو</label>
                    <textarea {...register('seo_description')} rows={3} className="w-full px-4 py-2 bg-surface border border-border rounded-input focus:outline-none focus:ring-2 focus:ring-primary resize-none" />
                  </div>
                </div>
              </div>
