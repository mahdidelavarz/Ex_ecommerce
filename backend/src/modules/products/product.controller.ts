...
export class ProductController {
  ...
  getRelated = asyncHandler(async (req: Request, res: Response) => {
    const products = await this.service.getRelated(req.params.slug, parseInt(req.query.limit as string) || 8);
    ApiResponseHelper.success(res, products);
  });

  getFilters = asyncHandler(async (req: Request, res: Response) => {
    const filters = await this.service.getFilters(req.query.category_id as string);
