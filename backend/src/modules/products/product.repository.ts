...
export class ProductRepository {
  ...
  async findAll(options: ProductQueryParams) {
    ...
    if (options.is_active !== undefined) {
      qb.andWhere('product.is_active = :is_active', { is_active: options.is_active });
    }

    if (options.is_public !== undefined) {
      qb.andWhere('product.is_public = :is_public', { is_public: options.is_public });
    }
    ...
    if (options.brand_id) {
      qb.andWhere('product.brand_id = :brand_id', { brand_id: options.brand_id });
    }
    ...
    if (options.min_price !== undefined || options.max_price !== undefined) {
      qb.having('MIN(variants.price) >= :minPrice', { minPrice: options.min_price || 0 });
      if (options.max_price) {
        qb.andHaving('MIN(variants.price) <= :maxPrice', { maxPrice: options.max_price });
      }
    }
    ...
    return {
      data: data.map((p: any) => ({
        ...
        brand: p.brand ? { id: p.brand.id, name: p.brand.name, slug: p.brand.slug, logo: p.brand.logo } : null,
        thumbnail: p.thumbnail || null,
        price_range: { min: parseFloat(p.min_price) || 0, max: parseFloat(p.max_price) || 0 },
  ...
  async findBySlug(slug: string) {
    const product = await this.repo.findOne({
      ...
      relations: ['category', 'brand', 'images', 'variants', 'variants.images', 'variants.variant_attribute_values', 'variants.variant_attribute_values.attribute_value', 'variants.variant_attribute_values.attribute_value.attribute', 'product_tags', 'product_tags.tag'],
    });
  ...
  async getFilters(categoryId: string) {
    ...
    const brandsQb = this.brandRepo
      ...
      .select(['brand.id', 'brand.name', 'brand.slug', 'brand.logo', 'COUNT(product.id) as count'])
    ...
    try {
      ...
      if (dto.tag_ids?.length) {
        const productTags = dto.tag_ids.map((tagId) =>
          this.productTagRepo.create({ product_id: savedProduct.id, tag_id: tagId })
        );
  ...
  async addImage(productId: string, dto: { image_url: string; alt_text?: string; is_thumbnail?: boolean; sort_order?: number }) {
    const product = await this.repo.findOne({ where: { id: productId } });
    if (!product) throw new NotFoundError('محصول یافت نشد');

    if (dto.is_thumbnail) {
      await this.imageRepo.update({ product_id: productId }, { is_thumbnail: false });
    }
  ...
  private async generateUniqueSlug(title: string, excludeId?: string): Promise<string> {
    ...
    let uniqueSlug = slug;
    while (true) {
      const qb = this.repo.createQueryBuilder('product').where('product.slug = :slug', { slug: uniqueSlug });
  ...
  private async getCategoryChildrenIds(categoryId: string): Promise<string[]> {
    const children = await this.categoryRepo.find({ where: { parent_id: categoryId } });
    let ids: string[] = [];
  ...
  private formatProductDetail(product: Product): any {
    const variants = product.variants?.map((v) => ({
      ...
      images: v.images?.map((img) => ({ id: img.id, image_url: img.image_url })) || [],
    })) || [];
    ...
    return {
      ...
      variants,
      tags: product.product_tags?.map((pt) => ({
        id: pt.tag.id,
        name: pt.tag.name,
        slug: pt.tag.slug,
      })) || [],
